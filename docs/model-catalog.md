...
服务器同步代码、上传固定权重及配置 VLM 密钥见 [手工部署更新](manual-update.md)。
...
| YOLOv26n（YOLO26n） | 林朴   | `pcb-yolov26n-20260912.pt` |
...
YOLOv11、YOLOv12 使用本机已有交付文件，复制并校验后统一名称。v11 SHA-256 与历史清单一致，旧清单文件大小已更正。2026-09-15 收到林朴同学的根目录 `best.pt`，确认架构为 YOLO26n、六类名称与系统一致，按 checkpoint 内记录的 2026-09-12 日期复制为固定版本。其训练参数为 100 轮，但导出 checkpoint 的 epoch 为 -1，不据此断言实际完成轮数。校验值及来源见 `backend/weights.manifest.json`，其中 source 相对仓库根目录，file 相对 backend 目录。
...
2026-09-15 对新交付 YOLO26n 使用本地 Windows CPU、Ultralytics 8.4.143 和真实 PCB 图片执行 API 验证，任务 succeeded、is_mock=false、返回 0 个检测框，JSON 导出一致。加载 663.81ms，推理 5562.86ms（单次首次运行，不作为性能基准）。证据：`backend/data/model-verification-20260915/9dc6792e060b4d1eb046e8b7da8cd53e/result.json`。尚不代表 Ubuntu 服务器验收通过或检测准确率合格。
